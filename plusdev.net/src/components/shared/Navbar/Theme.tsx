...
const Theme = ({ type = 'dropdown' }: { type?: string }) => {
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger className='p-1'>{mode === 'light' ? <LucideSun className='h-6 w-6 text-red-500' /> : <LucideMoon className='h-6 w-6 text-red-500' />}</DropdownMenuTrigger>
				...
				{menuItems.map((item: IMenuItems) => {
					return (
						<DropdownMenuItem
							...
							{item.icon} <span className={cn('text-zinc-800 dark:text-zinc-50', mode === item.mode && 'text-red-500 dark:text-red-400')}>{item.title}</span>
						</DropdownMenuItem>
					);
				})}
			</DropdownMenuContent>
