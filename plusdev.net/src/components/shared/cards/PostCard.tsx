...
const PostCard = ({ id, title, url, image, cretedAt, commentsCount, viewsCount, author, tags, upvotesCount, classes }: Props) => {
	return (
		<div className={cn('bg-zinc-100 dark:bg-zinc-900 rounded-[10px] p-9', classes)}>
			...
			<div className='flex flex-col-reverse items-start justify-between gap-5 sm:flex-row'>
				<div>
					...
					<Link to={url}>
						<h3 className='sm:h3-semibold line-clamp-2 text-zinc-900 dark:text-zinc-200 base-medium flex-1 transition-colors hover:text-red-500'>{title}</h3>
					</Link>
				</div>
			</div>
