...
const Metric = ({ value, type, title, textStyles, href, isAuthor, imgUrl }: Props) => {
	const iconComponent = useMemo(() => (!isAuthor ? iconMapping[type as keyof typeof iconMapping] : null), [type, isAuthor]);
		...
		return (
			<div>
				<Link
					...
					<span className={cn('flex items-center gap-1 text-zinc-800 dark:text-zinc-400 transition-colors hover:text-red-400', textStyles)}>
						{value}
	...
	return (
		<div className='flex flex-wrap items-center justify-center gap-1'>
			<span className={cn('flex items-center gap-1', textStyles)}>
				{iconComponent}
				{value}
				{type !== 'asked' && <span className='small-regular line-clamp-1'>{title}</span>}
			</span>
		</div>
	);
};
